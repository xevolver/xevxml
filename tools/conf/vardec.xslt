...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template match="xev_pragma[xev_target/SgGlobal]">
	...
	<xsl:apply-templates select="xev_clause_list"/>
  ...
  <xsl:template match="SgVarRefExp[@lvalue]">
    <xsl:copy>
      <xsl:copy-of select="@name"/>
    </xsl:copy>
  </xsl:template>

  <xsl:template match="SgFunctionRefExp[@samefile]">
    <xsl:copy>
      <xsl:copy-of select="@name"/>
  ...
  <xsl:template match="SgExprListExp[@samefile]">
    <xsl:copy>
      <xsl:apply-templates select="*"/>
    </xsl:copy>
  </xsl:template>
  ...
  <xsl:template match="SgInitializedName[count(SgArrayType)=0 and count(SgPointerType/SgArrayType)=0][count(SgAssignInitializer)=0][parent::SgVariableDeclaration]">
    <xsl:if test="count(preceding-sibling::*) - count(preceding-sibling::xev_pragma) = 0">
  ...
  <xsl:template match="SgVariableDeclaration">
	...
	<xsl:if test="(number(@type_modifier) mod 64) - (number(@type_modifier) mod 32) =  32">
	  <xsl:element name="SgFunctionCallExp">
		...
		<xsl:element name="SgActualArgumentExpression">
		  ...
		  <xsl:attribute name="samefile">0</xsl:attribute>
		  <xsl:element name="SgStringVal">
	...
	<xsl:if test="@storage_modifier = '2'">
      ...
      <xsl:apply-templates select="SgInitializedName"/>
    </xsl:copy>
  </xsl:template>
  ...
  <xsl:template match="SgActualArgumentExpression">
    <xsl:element name="SgAssignOp">
      <xsl:element name="SgVarRefExp">
	<xsl:attribute name="name"><xsl:value-of select="@name"/></xsl:attribute>
      </xsl:element>
