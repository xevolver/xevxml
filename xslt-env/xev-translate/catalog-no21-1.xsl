	...
	<xsl:template match="SgFortranDo">
		<xsl:choose>
			...
			<xsl:when test="preceding-sibling::*[1]/SgPragma/DIRECTIVE[@name='blocking']/CLAUSE[@name='block']">
				<xsl:param name="var1_nam" select="preceding-sibling::*[1]/SgPragma/DIRECTIVE[@name='blocking']/CLAUSE[@name='block']/LI[1]/@value"/>
				<xsl:param name="var1_siz" select="preceding-sibling::*[1]/SgPragma/DIRECTIVE[@name='blocking']/CLAUSE[@name='block']/LI[2]/@value"/>
				<xsl:param name="var1_lop" select="concat($var1_nam,$var1_nam)"/>
				<xsl:param name="var2_nam" select="preceding-sibling::*[1]/SgPragma/DIRECTIVE[@name='blocking']/CLAUSE[@name='block']/LI[3]/@value"/>
				<xsl:param name="var2_siz" select="preceding-sibling::*[1]/SgPragma/DIRECTIVE[@name='blocking']/CLAUSE[@name='block']/LI[4]/@value"/>
  				...
  				<xsl:variable name="tmp">
					<SgFortranDo end="1" nlabel="" slabel="" style="0">
						<SgAssignOp>
							<SgVarRefExp>
								<xsl:attribute name="name">
									<xsl:value-of select="preceding-sibling::*[1]/SgPragma/DIRECTIVE[@name='loop-roll']/CLAUSE[@name='dim']/LI[2]/@value"/>
								</xsl:attribute>
							</SgVarRefExp>
							<SgIntVal>
								<xsl:attribute name="value">
									<xsl:value-of select="preceding-sibling::*[1]/SgPragma/DIRECTIVE[@name='loop-roll']/CLAUSE[@name='dim']/LI[3]/@value"/>
								</xsl:attribute>
							</SgIntVal>
						</SgAssignOp>
						<SgVarRefExp>
							<xsl:attribute name="name">
								<xsl:value-of select="preceding-sibling::*[1]/SgPragma/DIRECTIVE[@name='loop-roll']/CLAUSE[@name='dim']/LI[4]/@value"/>
							</xsl:attribute>
						</SgVarRefExp>
						<SgNullExpression/>
					        <SgBasicBlock>
							<xsl:apply-templates select="." mode="loop-roll">
								<xsl:with-param name="idx" select="preceding-sibling::*[1]/SgPragma/DIRECTIVE[@name='loop-roll']/CLAUSE[@name='dim']/LI[1]/@value"/>
								<xsl:with-param name="var" select="preceding-sibling::*[1]/SgPragma/DIRECTIVE[@name='loop-roll']/CLAUSE[@name='dim']/LI[2]/@value"/>
							</xsl:apply-templates>
					        </SgBasicBlock>
					</SgFortranDo>
  				</xsl:variable>
	...
	<xsl:template match="SgPragmaDeclaration">
		<xsl:choose>
			<!--	!$xev blocking block(k,8,i,8)
				...
				ディレクティブを削除する
			-->
			<xsl:when test="./SgPragma/DIRECTIVE[@name='blocking']/CLAUSE[@name='block'] and not(contains(./SgPragma/DIRECTIVE[@name='blocking']/CLAUSE[@name='block']/LI[1]/@value,'default'))">
				<xsl:comment>
			...
			<!--	!$xev loop-roll dimp
				...
				ディレクティブを削除する
			-->
			<xsl:when test="./SgPragma/DIRECTIVE[@name='loop-roll']/CLAUSE[@name='dim'] and not(contains(./SgPragma/DIRECTIVE[@name='loop-roll']/CLAUSE[@name='dim']/LI[1]/@value,'default'))">
				<xsl:comment>
					SgPragmaDeclaration loop-roll
				</xsl:comment>
			</xsl:when>
