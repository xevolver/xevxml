...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="SgFortranDo">
    <xsl:choose>
      <xsl:when test="preceding-sibling::*[1]/SgPragma/@pragma = 'xev loop_tag'">
        <xsl:comment>xev loop_tag start test-3.xsl</xsl:comment>

        <xsl:variable name="step1">
          <xsl:apply-templates select="." mode="call_chill_unroll_jam"/>
        </xsl:variable>

        <xsl:apply-templates select="exslt:node-set($step1)" mode="call_chill_unroll"/>
      </xsl:when>
  ...
  <xsl:template match="SgPragmaDeclaration" mode="call_chill_unroll_jam">
  </xsl:template>
  <xsl:template match="PreprocessingInfo" mode="call_chill_unroll_jam">
  </xsl:template>
