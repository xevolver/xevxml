...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="SgFortranDo">
		<xsl:choose>
			<xsl:when
				test="preceding-sibling::*[1]/SgPragma/@pragma = 'xev loop_tag1'">
				<xsl:apply-templates mode="call_chill_unroll0"
					select="." />
			</xsl:when>
			<xsl:otherwise>
				<xsl:copy>
	...
	<xsl:template match="SgIfStmt">
		<xsl:choose>
			<xsl:when
				test="preceding-sibling::*[1]/SgPragma/@pragma = 'xev loop_tag2'">
				return 0;
			</xsl:when>
			<xsl:otherwise>
				<xsl:copy>
